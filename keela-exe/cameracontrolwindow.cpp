...
Keela::CameraControlWindow::CameraControlWindow(const guint id,
  ...
  const auto range_frame =
      Gtk::make_managed<Keela::FrameBox>("Range", Gtk::ORIENTATION_VERTICAL);
  ...
  range_check.signal_toggled().connect(
      sigc::mem_fun(*this, &CameraControlWindow::on_range_check_toggled));
  ...
  gain_spin.m_spin.signal_value_changed().connect(
      sigc::mem_fun(*this, &CameraControlWindow::on_gain_changed));
  ...
  rotation_combo.m_combo.signal_changed().connect(
      sigc::mem_fun(*this, &CameraControlWindow::on_rotation_changed));
  ...
  flip_horiz_check.signal_toggled().connect(
      sigc::mem_fun(*this, &CameraControlWindow::on_flip_horiz_changed));
  v_container.add(flip_horiz_check);
  flip_vert_check.signal_toggled().connect(
      sigc::mem_fun(*this, &CameraControlWindow::on_flip_vert_changed));
  ...
  fetch_image_button.signal_clicked().connect(
      sigc::mem_fun(*camera_manager->camera_stream_even->snapshot,
                    &Keela::SnapshotBin::take_snapshot));
...
void Keela::CameraControlWindow::set_resolution(const int width,
                                                const int height) {
...
void Keela::CameraControlWindow::update_split_frame_state(
    bool should_split_frames) {
...
std::vector<std::shared_ptr<Keela::ITraceable>>
Keela::CameraControlWindow::get_traces() {
...
void Keela::CameraControlWindow::update_traces() {
  ...
  auto even_trace = std::make_shared<CameraTrace>(
      camera_manager->camera_stream_even->get_trace_bin(), trace_gizmo_even,
      even_name);
  ...
  if (camera_manager->is_frame_splitting_enabled() && trace_gizmo_odd) {
    auto odd_trace = std::make_shared<CameraTrace>(
        camera_manager->camera_stream_odd->get_trace_bin(), trace_gizmo_odd,
...
void Keela::CameraControlWindow::set_trace_bin_framerate_caps(guint fps) {
  spdlog::info("Applying trace framerate {} to all traces for camera {}", fps,
               id);
  for (const auto &trace : m_traces) {
...
void Keela::CameraControlWindow::add_split_frame_ui() {
  ...
  if (rotation == ROTATION_90 || rotation == ROTATION_270) {
    frame_widget_odd = std::make_unique<VideoPresentation>(
        "Odd Frames", camera_manager->camera_stream_odd->presentation, *this,
        480, // width
        640  // height
    );
  } else {
...
void Keela::CameraControlWindow::update_gain_range() {
  ...
  if (min_gain == 0.0 && max_gain == 0.0) {
    gain_spin.m_spin.set_sensitive(false);
    spdlog::warn(
        "Gain control not supported by camera - disabling gain control UI");
    return;
  }
  // update the gain spin with the new range
  gain_spin.m_spin.set_adjustment(
      Gtk::Adjustment::create(min_gain, min_gain, max_gain, 1.0));
  gain_spin.m_spin.set_sensitive(true);

  spdlog::info("Updated gain control range to {:.1f} - {:.1f} dB", min_gain,
               max_gain);
}
...
void Keela::CameraControlWindow::update_exposure_time_range() {
  ...
  if (min_exposure_time == 0.0 && max_exposure_time == 0.0) {
    exposure_time_spin.m_spin.set_sensitive(false);
    spdlog::warn("Exposure time control not supported by camera - disabling "
                 "exposure time control UI");
    return;
  }
  // update the exposure time spin with the new range
  exposure_time_spin.m_spin.set_adjustment(Gtk::Adjustment::create(
      min_exposure_time, min_exposure_time, max_exposure_time, 1000.0));
  exposure_time_spin.m_spin.set_sensitive(true);

  spdlog::info("Updated exposure time control range to {:.1f} - {:.1f} μs",
