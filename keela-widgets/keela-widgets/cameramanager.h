...
public:
  ...
  std::shared_ptr<CameraStreamBin> camera_stream_even =
      std::make_shared<CameraStreamBin>("camera_stream_even");
  std::shared_ptr<CameraStreamBin> camera_stream_odd =
...
private:
  ...
  void remove_probe_by_id(gulong &probe_id, GstPad *pad,
                          const std::string &probe_name);

  // Frame filtering callback
  static GstPadProbeReturn
  frame_parity_probe_cb(GstPad *pad, GstPadProbeInfo *info, gpointer user_data);
