...
// should we use HSLuv for color mapping or should we use HSLuv?
uniform bool use_hsluv = false;
...
void main() {
    ...
    if (heatmap_enabled && texSample.x >= heatmap_threshold_low && texSample.x <= heatmap_threshold_high) {
        ...
        if (use_hsluv)
        {
            color = hsluvToRgb(H, heatmap_saturation * 255, heatmap_lightness * 255);
        } else {
            color = hsl2rgb(H);
        }
